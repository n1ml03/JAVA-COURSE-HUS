...
public class BreadStore {
	...
	public BreadStore() {
		// Sử dụng ArrayList thay vì LinkedList 
		// vì LinkedList không hiệu quả cho việc truy cập ngẫu nhiên
	...
	public boolean sell(Bread bread) {
		Iterator<Bread> iterator = breads.iterator(); // Sử dụng Iterator để cải thiện hiệu suất
		while (iterator.hasNext()) {
			Bread currentBread = iterator.next();
			if (currentBread.cost() == bread.cost()) {
				iterator.remove(); // Sử dụng iterator.remove() để xóa phần tử an toàn hơn
				return true;
			}
		}
		return false;
	...
	public List<Bread> filter(int howMany, boolean order) {
		...
		return breadList.subList(0, Math.min(howMany, breadList.size()));
	}
	...
	public static void main(String[] args) {
		...
		List<Bread> lowCostBreads = breadStore.filter(10, false);
